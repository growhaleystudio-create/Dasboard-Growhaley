...
type CaptureResponse = import('./scrape-session.js').CaptureResponse;
...
(function () {
  ...
  const buttonEl = document.getElementById('capture') as HTMLButtonElement | null;
  const debugEl = document.getElementById('debug') as HTMLElement | null;
  const maxItemsEl = document.getElementById('max-items') as HTMLSelectElement | null;
  if (!statusEl || !buttonEl || !maxItemsEl) return;
  const maxItemsSelect = maxItemsEl;

  function setStatusText(status: CaptureResponse | undefined): void {
    if (!statusEl) return;
    if (!status) {
  ...
  function persistMaxItems(value: string): void {
    if (!ALLOWED_MAX_ITEMS.has(value)) return;
    try {
  ...
  chrome.runtime.sendMessage({ type: 'leadsgen:popup-status' }, (response: CaptureResponse) => {
