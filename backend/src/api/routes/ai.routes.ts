...
export const aiRoutes =
  (deps: AiRoutesDeps): FastifyPluginAsync =>
  (fastify) => {
    fastify.get(
      '/usage',
      {
    ...
    fastify.put(
      ...
      async (request, reply) => {
        ...
        if (!parseResult.success) {
          throw appError({
            code: 'VALIDATION',
            messages: parseResult.error.errors.map((error) => error.message),
          });
        }
          ...
          if (input.apiBaseUrls !== undefined) {
            if (input.apiBaseUrls.text !== undefined) {
              await Promise.all([
                ...
                deps.settings.setApiBaseUrl(
                  params.id,
                  input.apiBaseUrls.text,
                  'content_suggestion',
                ),
              ]);
            }
            if (input.apiBaseUrls.imageGeneration !== undefined) {
              await deps.settings.setApiBaseUrl(
                params.id,
                input.apiBaseUrls.imageGeneration,
                'image_generation',
              );
            }
          }
    ...
    fastify.post(
      ...
      async (request, reply) => {
        ...
        try {
          const result = await deps.reanalyze.recomputeScore(params.id, params.leadId);
          ...
          request.log.error(
            ...
            'Lead score recompute request failed',
          );
          throw error;
        }
      },
    ...
    fastify.post(
      '/leads/:leadId/regenerate-ai-insight',
      {
        preHandler: [
          fastify.requireAuth,
          fastify.requireTeamId,
      ...
      async (request, reply) => {
        const params = request.params as { id: string; leadId: string };
        try {
          const result = await deps.reanalyze.regenerateAiInsight(
            params.id,
            params.leadId,
          ...
          request.log.error(
            ...
            'AI insight regeneration request failed',
          );
          throw error;
        }
      },
    ...
    fastify.post(
      ...
      async (request, reply) => {
        const params = request.params as { id: string; leadId: string };
        try {
          const scoreResult = await deps.reanalyze.recomputeScore(params.id, params.leadId);
          if (!scoreResult.ok) throw appError(scoreResult.error);
          const insightResult = await deps.reanalyze.regenerateAiInsight(
            params.id,
            params.leadId,
            request.session!.userId,
          );
          if (!insightResult.ok) throw appError(insightResult.error);
          reply.header('Deprecation', 'true');
          return reply.status(200).send({
            leadId: params.leadId,
            scoringState: scoreResult.value.scoringState,
            aiState: insightResult.value.aiState,
          });
        } catch (error) {
          request.log.error(
            {
            ...
            'Legacy lead reanalyze request failed',
          );
          throw error;
        }
      },
    ...
    fastify.get(
      ...
      async (request) => {
        ...
        const apiKey = await deps.settings.loadApiKey(
          params.id,
          purpose === 'image' ? 'image_generation' : 'leads',
        );
        const rawBaseUrl = await deps.settings.loadApiBaseUrl(
          params.id,
          purpose === 'image' ? 'image_generation' : 'leads',
        );
        ...
        try {
          ...
          if (providerKind === 'google') {
            ...
            const url = `${rawBaseUrl}/v1beta/models`;
