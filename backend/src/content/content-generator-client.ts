...
- Gunakan 2-4 emoji secara strategis
...
export class ContentGeneratorClient {
  ...
  async generateText(
    ...
    let systemInstruction = 'Anda adalah spesialis pembuat konten pemasaran digital profesional.';
    ...
    const finalPrompt = [
      ...
      options.imageInstruction ? `Instruksi Visual (sebagai konteks): "${options.imageInstruction}"` : ''
    ].filter(Boolean).join('\n\n');
    ...
    const fetchBody = providerKind === 'openai_compatible'
      ? {
          ...
          messages: [
            ...
            {
              role: 'user',
              content: template?.referenceImages && template.referenceImages.length > 0
                ? [
                    { type: 'text', text: 'Analisis gambar-gambar referensi untuk memahami nada penulisan.' },
                    ...template.referenceImages.map(img => ({ type: 'image_url', image_url: { url: img } })),
  ...
  async generateImage(
    ...
    if (template) {
      ...
      if (referenceDescription) parts.push(`incorporate this visual style from references: ${referenceDescription}`);
      if (style?.mood) parts.push(`${style.mood} mood`);
      if (style?.brandColors?.length) parts.push(`using brand colors: ${style.brandColors.join(', ')}`);
    ...
    try {
      // Use AbortController for a 60-second timeout to prevent hanging requests
      ...
      const fetchBody = providerKind === 'openai_compatible'
        ? {
            ...
            ...(shouldRequestOpenAiImageResponseFormat(this.imageModel) ? { response_format: 'b64_json' } : {}),
          }
        : {
      ...
      if (response.ok) {
        ...
        const errorText = await response.text();
        console.warn(`Imagen API failed, status: ${response.status}. Error: ${errorText}. Falling back to curated image.`);
      }
    } catch (err: any) {
      if (err.name === 'AbortError') {
        console.warn('Imagen generation timed out after 60s. Falling back to curated stock image.');
      } else {
        console.warn(`Imagen generation failed: ${err.message}. Falling back to curated stock image.`);
      }
    }
  ...
  private async describeReferenceImages(imageUrls: string[], signal?: AbortSignal): Promise<string> {
    const baseUrl = requireProviderBaseUrl(this.apiBaseUrl);
    const providerKind = providerKindFromBaseUrl(baseUrl);
    const url = providerKind === 'openai_compatible'
      ? `${baseUrl}/v1/chat/completions`
          ...
          messages: [
            {
              role: 'user',
              content: [
                { type: 'text', text: 'Analyze these reference images. Describe the visual style, lighting, composition, mood, and any recurring visual motifs. Provide a concise comma-separated list of visual descriptors.' },
...
async function generateFallbackImage(prompt: string, template?: ContentTemplate): Promise<string> {
  ...
  const curatedImages = [
    { keys: ['coffee', 'cafe', 'roaster', 'barista'], url: 'https://images.unsplash.com/photo-1507133750040-4a8f57021571?q=80&w=800&auto=format&fit=crop' },
    { keys: ['tech', 'software', 'app', 'coding', 'web'], url: 'https://images.unsplash.com/photo-1498050108023-c5249f4df085?q=80&w=800&auto=format&fit=crop' },
    { keys: ['law', 'lawyer', 'legal', 'court'], url: 'https://images.unsplash.com/photo-1589829545856-d10d557cf95f?q=80&w=800&auto=format&fit=crop' },
    { keys: ['clinic', 'doctor', 'health', 'medical', 'hospital'], url: 'https://images.unsplash.com/photo-1505751172876-fa1923c5c528?q=80&w=800&auto=format&fit=crop' },
    { keys: ['gym', 'fitness', 'workout', 'sport'], url: 'https://images.unsplash.com/photo-1517838277536-f5f99be501cd?q=80&w=800&auto=format&fit=crop' },
    { keys: ['restaurant', 'food', 'dine', 'eating'], url: 'https://images.unsplash.com/photo-1517248135467-4c7edcad34c4?q=80&w=800&auto=format&fit=crop' },
    { keys: ['hotel', 'stay', 'travel', 'room'], url: 'https://images.unsplash.com/photo-1566073771259-6a8506099945?q=80&w=800&auto=format&fit=crop' },
    { keys: ['business', 'marketing', 'sales', 'corporate', 'meeting'], url: 'https://images.unsplash.com/photo-1460925895917-afdab827c52f?q=80&w=800&auto=format&fit=crop' }
  ];
